...
#                                                              SUBMISSION VIEWS
    ...
    def test_successful_homework_submission(self):
        ...
        response = self.client.post(reverse(
            'homeworks'), homework)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Home work added successfully!!!!',
                      response.content)
...
#                                                                         VIEWS
    def test_delete_notes(self):
        ...
        response = self.client.delete(reverse('delete-notes',
...
#                                                               DELETE VIEWS
    ...
    def test_delete_homework(self):
        '''
        Tests that a the view deletes the created data and is redirected
        to another page
        '''
        new_hw = Homework.objects.create(title='This is a testdata.',
                                         description='It is test description',
                                         subject='Test subject',
                                         due_date='2024-02-21 12:00:00',
                                         is_finished=False)
        response = self.client.delete(reverse('delete-homework',
                                      args=[new_hw.id]), follow=True)
        self.assertEqual(response.status_code, 200)
...
#                                                                         VIEWS
    ...
    def test_delete_todo_list(self):
        ...
        new_to_do = Todo.objects.create(title='This is a testdata.',
                                        is_completed=False)
        response = self.client.delete(reverse('delete-todo',
                                      args=[new_to_do.id]), follow=True)
        self.assertEqual(response.status_code, 200)
